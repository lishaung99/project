...
public:
    SkipList(int max_level, Compare comp = Compare());          // 构造函数
    ~SkipList();            // 类拥有的析构函数
...
SkipList<K, V, Compare>::SkipList(int max_level, Compare comp)
    :_max_level{max_level},_skip_list_level{0},_element_count{0},_header{nullptr},_comp{comp}
{
    K k;
    V v;
    //  创建头节点，并初始化键值为默认值
...
bool SkipList<K, V, Compare>::search_element(K key){
    ...
    for(int i = _skip_list_level; i >= 0; i--){
        ...
        while( current->forward[i] && _comp(current->forward[i]->get_key(), key)){
            // 移动到当前层的下一个节点
    ...
    if(current && !_comp(current->get_key(), key) && !_comp(key, current->get_key())){
        // 找到匹配值
...
int SkipList<K, V, Compare>::insert_element(const K key, const V value ){
    ...
    for(int i = _skip_list_level; i >= 0; i--){
        ...
        while(current->forward[i] != nullptr && _comp(current->forward[i]->get_key(), key)){
            current = current->forward[i];
        }
    ...
    current = current->forward[0]; 
    // 检查待插入的节点的键是否已存在 
    if (current != NULL && !_comp(current->get_key(), key) && !_comp(key, current->get_key())) {
        // 键已存在，取消插入
    ...
    if (current == NULL || _comp(key, current->get_key())) {
        // 通过随机函数决定新节点的层级高度
...
// 修改节点的值
template<typename K, typename V, typename Compare>
bool SkipList<K, V, Compare>::modify_value(K key, V new_value) {
    mtx.lock();  // 加锁
    Node<K, V>* current = _header;
    ...
    // 确认找到了待修改的节点
    if (current != nullptr && !_comp(current->get_key(), key) && !_comp(key, current->get_key())) {
