...
<template>
  <NModal title="提示词商店" v-model:show="show" style="width: 90%; max-width: 900px;" preset="card" content-class="modal-content-bg" >
    <div class="space-y-4 "> <!-- chat-silder-footer-->
      <NTabs >
        <NTabPane name="local">
          <template #tab>
            <SvgIcon class="text-lg" icon="ri-map-pin-line" />
            <span class="ml-2">{{ $t('store.local') }}</span>
          </template>
          <div
        ...
        <NTabPane name="download">
          <template #tab>
            <SvgIcon class="text-lg" icon="ri-cloud-line" />
            <span class="ml-2">{{ $t('store.online') }}</span>
          </template>
          <p class="mb-4">
